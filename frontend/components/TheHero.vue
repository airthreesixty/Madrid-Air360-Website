...
import { allHerosQuery } from '~~/graphql/queries'

const { data } = await useAsyncQuery(allHerosQuery)

const heroes = ref([])

if (data.value?.heroes) {
  heroes.value = data.value.heroes.data
  console.log(data)
} else {
  // console.log('Fetch failed Hero')
  // console.log(data)
}
</script>
